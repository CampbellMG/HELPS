...
import { Message, makeMockMessage } from '../../types/model/Message';
    ...
    switch (action.type) {
        case (MessageActionTypes.RECEIVE):
            if (isUndefined(action.messages)) {
                ...
                let selectedMessage: Message = action.messages.length > 0 ? action.messages[0] : { id: Number.MAX_SAFE_INTEGER, title: 'Undefined title', content: 'Undefined content' };
